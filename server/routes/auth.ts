...
export const handleRegister: RequestHandler = (req, res) => {
  try {
    const { firstName, lastName, email, rollNumber, department, semester, password } = req.body;
    ...
    const existingUser = users.find(user => user.email === email || user.rollNumber === rollNumber.toUpperCase());
    if (existingUser) {
      return res.status(400).json({
        success: false,
