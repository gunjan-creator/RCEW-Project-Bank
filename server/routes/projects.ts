...
export const handleCreateProject: RequestHandler = (req, res) => {
  try {
    ...
    // Get user info from JWT token (simulated for now with mock user)
    const token = req.headers.authorization?.replace("Bearer ", "");
    let authorId = "1";
    let author = "Demo User";

    // In a real app, decode JWT token and get user info from database
    // For now, we'll use mock data but in proper format
    if (token) {
      // Mock user data - in real app, decode JWT and query database
      const mockUsers: Record<
        string,
        { id: string; firstName: string; lastName: string }
      > = {
        "mock-token": { id: "1", firstName: "Tamanna", lastName: "Yadav" },
        "user-token": { id: "2", firstName: "Gunjan", lastName: "Singla" },
        "test-token": { id: "3", firstName: "Gargi", lastName: "Kathait" },
        "demo-token": { id: "4", firstName: "Tanya", lastName: "Goyal" },
      };

      const user = mockUsers[token] || mockUsers["mock-token"];
      authorId = user.id;
